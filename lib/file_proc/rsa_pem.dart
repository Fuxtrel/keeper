...
class RsaKeyHelper {
  ...
  String encrypt(Uint8List plaintext, RSAPublicKey publicKey) {
    ...
    int lastIndex = 0;
    while (length > 255) {
      parts.add(plaintext.sublist(lastIndex, lastIndex + 255));
      lastIndex += 256;
      length -= 256;
    }
    parts.add(plaintext.sublist(lastIndex));
    // String encryptedPart = '';
    List<int> list = [];
    parts.forEach((element) {
      var cipherText = cipher.process(element);
      list.addAll(cipherText.toList());
    });

    Uint8List result = Uint8List.fromList(list);
    return new String.fromCharCodes(result);
  }

  Uint8List decrypt(String ciphertext, RSAPrivateKey privateKey) {
    var cipher = new RSAEngine()
      ..init(false, new PrivateKeyParameter<RSAPrivateKey>(privateKey));
    int length = ciphertext.length;
    List<Uint8List> parts = [];
    Uint8List inputList = Uint8List.fromList(ciphertext.codeUnits);
    int lastIndex = 0;
    int counter = 0;
    while (length > 255) {
      parts.add(inputList.sublist(lastIndex, lastIndex + 255));
      lastIndex += 256;
      length -= 256;
      counter++;
    }
    parts.add(inputList.sublist(lastIndex));
    counter++;
    // String encryptedPart = '';
    List<int> list = [];
