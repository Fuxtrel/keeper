...
void _receive(SendPort port) async {
  ReceivePort isolateRecievePort = ReceivePort();
  port.send(isolateRecievePort.sendPort);
  isolateRecievePort.listen((portList) {
    if (portList is List<String>) {
      var params = portList;
      try {
          ...
          if (decodeJson.containsKey("keepAlive") &&
            ...
            try {
              ...
              if (str[0] == '{') {
                ...
                if (!download) {
                  var isolate =
                      await Isolate.spawn(_partRecieve, port.sendPort);
                  isolateList.add(
                    RecievePartInfo(
                      isolate: isolate,
                      port: port,
